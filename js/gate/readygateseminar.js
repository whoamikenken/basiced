...
function get_result() {
    ...
    var form_data = {
        ...
        dvid: getDeviceId(),
        ip: $("input[name='privateip']").val()
    };

    $.ajax({
        url: $("#site_url").val() + "/fingerprint_/resultSeminarChecker",
        type: "POST",
        data: form_data, 
        success: function(response)
        {
            ...
            if (response == "wait") {
                ...
                $("#container-response").empty().css("color","red").html("<h2><b>Oh snap!</b> Something went wrong! Reloading gate..</h2>");
                setTimeout(function(){ 
                // location.reload();
            }, 2000);
            }else{
...
function get_rfid() {
    ...
    $.ajax({
        url: $("#site_url").val() + "/fingerprint_/rfidSeminarChecker",
        type: "POST",
        data: {
            tmp: $("#login-rfid").val(),
