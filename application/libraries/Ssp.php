...
class SSP {
    ...
    {
        $out = array();
        $data = Globals::resultarray_XHEP($data);
        for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
            $row = array();

            for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
                $column = $columns[$j];
                /*if(isset($data[$i]["fullname"])){
                    $data[$i]["fullname"] = Globals::_e($data[$i]["fullname"]);
                }
                if(isset($data[$i]["username"])){
                    $data[$i]["username"] = Globals::_e($data[$i]["username"]);
                }*/
    ...
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '')
    {
        ...
        if($joinQuery){
            ...
            $query =  "SELECT DISTINCT ".implode(", ", $col)."
			 $joinQuery
			 $where
			 $extraWhere
			 $groupBy
