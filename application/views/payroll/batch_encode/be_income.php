...
<div class="panel animated fadeIn delay-1s">
    ...
    <div class="panel-body emplist">
        <table class="table table-hover table-bordered datatable" id="be_income">
			...
			<tbody>
				<input type="text" name="incomeCode" id="incomeCode" value="<?= $codeincome ?>" hidden>
				<?php foreach($emplist as $employeeid => $emp_data): ?>
					<tr id="<?= $employeeid ?>">
						<td><?= $employeeid ?></td>
						<td><?= $emp_data['fullname']?></td>
						<td tag="new">
							<div class='input-group date' data-date="<?= $emp_data['datefrom'] ?>" data-date-format="yyyy-mm-dd">
						...
						<td tag="new">
							<select class="cutoff_period form-control" name="cutoff_period" id="cutoff_period" style="width: 100%">
								<?=$this->payrolloptions->quarter($emp_data['cutoff_period'],FALSE,$emp_data['schedule'],TRUE);?>
							</select>
						</td>
						<td><button type="button" class="btn btn-danger clearRow">Clear</button></td>
					</tr>
				<?php endforeach ?>
			</tbody>
		</table>
...
<script src="<?=base_url()?>js/batch_encode/be_income.js"></script>