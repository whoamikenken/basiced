			...
			<tr>	
				<td tag='deduct' class="align_center col-md-1">
                	<div class="btn-group">
                    	...
                    	<button type="button" class="btn btn-info" data-toggle="collapse" data-target="#content-other-<?php echo  $mrow->id ?>"><span class="glyphicon glyphicon-arrow-down"></span></button>
                    </div>
                </td>

                <td>
                    <b><?php echo  $mrow->description ?></b>
                    <div id="content-other-<?php echo  $mrow->id ?>" class="collapse">
						<table class="table table-hover table-striped table-bordered app_base_list_other">
							<thead>	
								<tr>	
									<th>Actions</th>
									<th>Code</th>
...
<script>
	...
	$(document).ready(function(){
	    var table = $('.app_base_list_other').dataTable({
		    "bPaginate": false,
		    "bLengthChange": false,
		    "bFilter": false,
		    "bAutoWidth": false 
		...
		$(document).ready(function(){
		    var table = $('#oth_request').DataTable({
		    });
