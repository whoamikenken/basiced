	...
	<table class="table-hover table-bordered table-striped datatable table-template" id="adj_table">
		<thead>
			<tr>
				...
				<th rowspan="2">OB DATE/S</th>
				<th rowspan="2" width="20px">TOTAL HOURS</th>
				<th rowspan="2">AMOUNT</th>
				<th colspan="<?=($arr_income_config > 0) ? sizeof($arr_income_config) : 2?>">INCOME ADJUSTMENT</th>
		...
		<tbody>
			...
			?>
							<tr>
								...
								<?php if($arr_income_config) {?>
									<? foreach ($arr_income_config as $code_income => $det) { ?>
											<td><?=isset($detail['income_adj'][$code_income]) ? number_format($detail['income_adj'][$code_income],2) : ''?></td>
									<? } ?>
								<?php }else{ ?>
									<td></td>
								<?php } ?>
								...
								<td class="align_center">
									<?if($status != 'PROCESSED'){?>
										<input type="checkbox" name="include" class="double-sized-cb" 
												employeeid="<?=$empid?>" request_id="<?=$detail['request_id']?>" 
												date="<?=$detail['date']?>" total_hours="<?=$detail['total_hours']?>" 
...
<script>
	...
	$('#save_process_adj').unbind('click').on('click',function(){
		...
		$('input[name=include]:checked').each(function(){
		    ...
		    det['total_hours'] = $(this).attr('total_hours');
		...
		$.ajax({
		 url     :   "<?=site_url("application_adj_/saveCorrectionAdj")?>",
		 type    :   "POST",
		 dataType : 'json',
		 data    :   form_data,
