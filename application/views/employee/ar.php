 ...
 * @copyright 2019
 */
$awards = $this->extras->showreportseduclevel(' - Select AWARD - ','AR');
$sm_date = date("Y-m-d");
?>
...
<div class="form-group">
  <label class="col-sm-4 control-label">Award Obtained</label>
  <div class="col-sm-7">
    <input type="text" name="ar_award" id="ar_award" class="form-control" value=""/>
    <!-- <select class="form-control" name="ar_award" id="ar_award">
      <?php foreach($awards as $aw => $award):?>
          <option <?=($aw==$awards ? " selected" : "")?> value="<?= $aw ?>"><?= $award ?></option>
      <?php endforeach;?>
    </select> -->
  </div>
</div>
...
<div class="form-group">
  <label class="col-sm-4 control-label">Granting Agency / Org</label>
  <div class="col-sm-7">
    <input type="text" name="ar_instituition" id="ar_instituition" class="form-control" value=""/>
  </div>
</div>
...
<div class="form-group">
  ...
  <div class="col-sm-7">
    <input type="text" name="ar_address" id="ar_address" class="form-control" value=""/>
  </div>
</div>

<div class="form-group">
  <label class="col-sm-4 control-label">Year Received</label>
  <div class="col-sm-7">
    <div class='input-group date date_issued' data-date="<?=date("Y-m-d",strtotime($sm_date))?>" data-date-format="yyyy-mm-dd">
        <input type='text' class="form-control" size="16" name="ar_datef"  id="ar_datef" value="<?=date("Y-m-d",strtotime($sm_date))?>"/>
...
<script>
    ...
    setTimeout(function(){
      ...
      if(typeof attr !== typeof undefined && attr !== false && attr != ''){
        ...
        $.ajax({
              ...
              data: {id:GibberishAES.enc( $("input[name='tbl_id']").val(), toks), table:GibberishAES.enc( 'employee_awardsrecog', toks), employeeid:GibberishAES.enc( $("#employeeid_").val(), toks) , toks:toks},
              dataType: "JSON",
    ...
    $.ajax({
         ...
         data : {table:'employee_awardsrecog', employeeid: $("#employeeid_").val(), baseid: $("input[name='tbl_id']").val()},
         dataType: 'JSON',
         type : "POST",
 ...
 var $validator = $("#myForm").validate({
  ...
  var fileName = filename = file = mime = ''; 
        ...
        if ($("#uploadFile").val() != "") {
            ...
            fd.append('award',  GibberishAES.enc($("input[name='ar_award']").val(), toks));
   ...
   if($("#myForm").valid()){
      ...
      if(cobj){
         // $(cobj).find("td:eq(0)").text($("#ar_award option:selected").text());
         // $(cobj).find("td:eq(0)").attr("relaward", $("select[name='ar_award']").val());  
         $(cobj).find("td:eq(0)").text($("input[name='ar_award']").val());
         $(cobj).find("td:eq(1)").text($("input[name='ar_instituition']").val());
         $(cobj).find("td:eq(2)").text($("input[name='ar_address']").val());         
         $(cobj).find("td:eq(3)").text($("input[name='ar_datef']").val());
         if ($("#uploadFile").val() != "") {
         ...
         var ntr = $("<tr></tr>");
         // $(ntr).append("<td relaward="+$("select[name='ar_award']").val()+">"+$("#ar_award option:selected").text()+"</td>");
         /*$(ntr).append("<td>"+$("input[name='sm_publication']").val()+"</td>"); */
         $(ntr).append("<td>"+$("input[name='ar_award']").val()+"</td>");
         $(ntr).append("<td>"+$("input[name='ar_instituition']").val()+"</td>");
         $(ntr).append("<td>"+$("input[name='ar_address']").val()+"</td>");
         $(ntr).append("<td>"+$("input[name='ar_datef']").val()+"</td>");
         ...
         if ("<?= $this->session->userdata("usertype") ?>" == "ADMIN" || "<?= $this->session->userdata("usertype") ?>" == "EMPLOYEE") {
           $("<div id='btn_pos'><a class='btn btn-warning delete_ar' tbl_id='"+ tbl_id +"'><i class='glyphicon glyphicon-trash'></i></a></div>").click(function(){
           ...
           $("<div id='btn_pos'><a class='btn btn-primary edit_ar' tbl_id='"+ tbl_id +"' href='#modal-view' data-toggle='modal' style='margin-right: 10px;'><i class='glyphicon glyphicon-edit'></i></a>").click(function(){
              addAward($(this).children(), tbl_id);
           }).appendTo($(mtd));
         }
         $(ntr).append($(mtd));
