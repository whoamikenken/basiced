...
<form id="form_education" class="form-horizontal">
  ...
  <label class="col-sm-4 control-label">Topic</label>
    <div class="col-sm-7">
      <input type="text" name="sm_topic" id="sm_topic" class="form-control" value=""/>
    </div>
  </div>

  <div class="form-group">
  <label class="col-sm-4 control-label">Organizer</label>
    <div class="col-sm-7">
  ...
  <label class="col-sm-4 control-label">Venue</label>
    <div class="col-sm-7">
      <input type="text" name="sm_venue" id="sm_venue" class="form-control" value=""/>
    </div>
  </div>
    ...
    setTimeout(function(){
      ...
      if(typeof attr !== typeof undefined && attr !== false && attr != ''){
        ...
        $.ajax({
              ...
              data: {id:GibberishAES.enc( $("input[name='tbl_id']").val(), toks), table:GibberishAES.enc( 'employee_resource', toks), employeeid:GibberishAES.enc( $("#employeeid_").val(), toks) , toks:toks},
              dataType: "JSON",
...
function getDataRequestDetails(){
    $.ajax({
         ...
         data : {table:'employee_resource', employeeid: $("#employeeid_").val(), baseid: $("input[name='tbl_id']").val()},
         dataType: 'JSON',
         type : "POST",
...
$(".button_save_modal").unbind("click").click(function(){
 ...
 var $validator = $("#form_education").validate({
   ...
   if($("#form_education").valid()){
      ...
      if(cobj){
         $(cobj).find("td:eq(0)").text($("input[name='sm_datef']").val());
         $(cobj).find("td:eq(1)").text($("input[name='sm_topic']").val());
         $(cobj).find("td:eq(2)").text($("input[name='sm_organizer']").val());  
         $(cobj).find("td:eq(3)").text($("input[name='sm_venue']").val());
         if ($("#uploadFile").val() != "") {
        ...
        /*save/update data first*/
         ...
         // $(ntr).append("<td educse='"+$("select[name='se_educlevel'] :selected").val()+"'>"+$("select[name='se_educlevel'] :selected").text()+"</td>");
         $(ntr).append("<td>"+$("input[name='sm_datef']").val()+"</td>");
         $(ntr).append("<td>"+$("input[name='sm_topic']").val()+"</td>");
         $(ntr).append("<td>"+$("input[name='sm_organizer']").val()+"</td>");
         $(ntr).append("<td>"+$("input[name='sm_venue']").val()+"</td>");
         ...
         $("<div id='btn_pos'><a class='btn btn-warning delete_resource' tbl_id='"+ tbl_id +"'><i class='glyphicon glyphicon-trash'></i></a></div>").click(function(){
         ...
         $("<div id='btn_pos'><a class='btn btn-primary edit_resource' tbl_id='"+ tbl_id +"' href='#modal-view' data-toggle='modal' style='margin-right: 10px;'><i class='glyphicon glyphicon-edit'></i></a>").click(function(){
            addResource($(this).children(), tbl_id);
         }).appendTo($(mtd));
         }
         $(ntr).append($(mtd));
         $(ntr).appendTo($("#resourceinfolist").find("tbody"));      
      }  
      $("#modalclose").click();
