<?php
$publicationType = $this->extras->showreportseduclevel('- Select PUBLICATION -','PUB');
/**
 * @author Justin
 * @copyright 2016
 */
...
<form id="myForm" class="form-horizontal">
  <div class="form-group">
    <label class="col-sm-4 control-label">Type of Publication</label>
    <div class="col-sm-7">
      <select class="form-control" name="sm_publication" id="sm_publication">
                <?php foreach($publicationType as $pt => $publication):?>
                    <option <?=($pt==$publicationType ? " selected" : "")?> value="<?= $pt ?>"><?= $publication ?></option>
                <?php endforeach;?>
      </select> 
    </div>
  </div>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-7">
      <input type="text" name="sm_title" class="form-control" id="sm_title" value=""/>
    </div>
  </div>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-7">
      <input type="text" name="sm_publisher" class="form-control" id="sm_publisher" value=""/>
    </div>
  </div>
  ...
  <div class="form-group">
    <label class="col-sm-4 control-label">Type of Authorship</label>
    <div class="col-sm-7">
      <input type="text" name="sm_type" id="sm_type" class="form-control" value=""/>
    </div>
  </div>
...
<script>
    ...
    setTimeout(function(){
      ...
      if(typeof attr !== typeof undefined && attr !== false && attr != ''){
        ...
        $.ajax({
              ...
              data: {id:GibberishAES.enc( $("input[name='tbl_id']").val(), toks), table:GibberishAES.enc( 'employee_pgd', toks), employeeid:GibberishAES.enc( $("#employeeid_").val(), toks) , toks:toks},
              dataType: "JSON",
    ...
    $.ajax({
         ...
         data : {table:'employee_pgd', employeeid: $("#employeeid_").val(), baseid: $("input[name='tbl_id']").val()},
         dataType: 'JSON',
         type : "POST",
...
$(".button_save_modal").unbind("click").click(function(){
 ...
 var fileName = filename = file = mime = ''; 
   ...
   if($("#myForm").valid()){
      ...
      if(cobj){
         /*$(cobj).find("td:eq(0)").text($("input[name='sm_publication']").val());*/
         $(cobj).find("td:eq(0)").text($("#sm_publication option:selected").text());
         $(cobj).find("td:eq(0)").attr("relpublication", $("select[name='sm_publication']").val()); 
         $(cobj).find("td:eq(1)").text($("input[name='sm_title']").val());
         $(cobj).find("td:eq(2)").text($("input[name='sm_publisher']").val());
         $(cobj).find("td:eq(3)").text($("input[name='sm_date']").val());
         $(cobj).find("td:eq(4)").text($("input[name='sm_type']").val());  
         if ($("#uploadFile").val() != "") {
      ...
      /*save/update data first*/
         ...
         var ntr = $("<tr></tr>");
         // $(ntr).append("<td>"+$("select[name='sm_publication']").val()+"</td>"); 
          $(ntr).append("<td relpublication='"+$("select[name='sm_publication']").val()+"'>"+$("#sm_publication option:selected").text()+"</td>");
         $(ntr).append("<td>"+$("input[name='sm_title']").val()+"</td>");
         $(ntr).append("<td>"+$("input[name='sm_publisher']").val()+"</td>");
         $(ntr).append("<td>"+$("input[name='sm_date']").val()+"</td>");
         $(ntr).append("<td>"+$("input[name='sm_type']").val()+"</td>");
         ...
         if ("<?= $this->session->userdata("usertype") ?>" == "ADMIN" || "<?= $this->session->userdata("usertype") ?>" == "EMPLOYEE") {
           $("<div id='btn_pos'><a class='btn btn-warning delete_pgd' tbl_id='"+ tbl_id +"'><i class='glyphicon glyphicon-trash'></i></a></div>").click(function(){
           ...
           $("<div id='btn_pos'><a class='btn btn-primary edit_pgd' href='#modal-view' data-toggle='modal' tbl_id='"+ tbl_id +"' style='margin-right: 10px;'><i class='glyphicon glyphicon-edit'></i></a>").click(function(){
              addPGD($(this).children(), tbl_id);
           }).appendTo($(mtd));
         }
         $(ntr).append($(mtd));
