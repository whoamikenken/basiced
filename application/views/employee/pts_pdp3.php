...
$titleLevel = $this->extras->showreportseduclevel(' - Select PSYCHOSOCIAL - CULTURAL - ','PTS_PDP3');
?>
...
<script>
    ...
    setTimeout(function(){
      ...
      if(typeof attr !== typeof undefined && attr !== false && attr != ''){
        ...
        $.ajax({
              ...
              data: {id:GibberishAES.enc( $("input[name='tbl_id']").val(), toks), table:GibberishAES.enc( 'employee_pts_pdp3', toks), employeeid:GibberishAES.enc( $("#employeeid_").val(), toks) , toks:toks},
              dataType: "JSON",
    ...
    $.ajax({
         ...
         data : {table:'employee_pts_pdp3', employeeid: $("#employeeid_").val(), baseid: $("input[name='tbl_id']").val()},
         dataType: 'JSON',
         type : "POST",
...
$(".button_save_modal").unbind("click").click(function(){
 ...
 var fileName = filename = file = mime = ''; 
   ...
   if($("#myForm").valid()){
      var cobj = "";
      $("#pts_pdp3infolist").find("tbody tr").each(function(){
      ...
      if(cobj){
        if($("#sm_title").val() == 'others'){
         ...
         // $(cobj).find("td:eq(4)").text($("input[name='sm_location']").text());
         if ($("#uploadFile").val() != "") {
 ...
 /*save/update data first*/
        ...
        if ("<?= $this->session->userdata("usertype") ?>" == "ADMIN" || "<?= $this->session->userdata("usertype") ?>" == "EMPLOYEE") {
           $("<div id='btn_pos'><a class='btn btn-warning delete_pts_pdp3' tbl_id='"+ tbl_id +"'><i class='glyphicon glyphicon-trash'></i></a></div>").click(function(){
           ...
           $("<div id='btn_pos'><a class='btn btn-primary edit_pts_ptp3' href='#modal-view' data-toggle='modal' tbl_id='"+ tbl_id +"' style='margin-right: 10px;'><i class='glyphicon glyphicon-edit'></i></a>").click(function(){
              addpts_pdp3($(this).children(), tbl_id);
           }).appendTo($(mtd));
         }
         $(ntr).append($(mtd));
