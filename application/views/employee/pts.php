...
$titleLevel = $this->extras->showreportseduclevel(' - Select POVEDAN SPIRITUAL and SPIRITUAL - ','PTS_PDP');
?>
...
<form id="myForm" class="form-horizontal">
  <div class="form-group">
    ...
    <div class="col-sm-7">
      <select class="form-control chosen" name="sm_title" id="sm_title" required>
...
<!--   <div class="form-group">
    ...
    <div class="col-sm-7">
      <select class="form-control chosen" name="sm_venue" id="sm_venue" required>
...
<script>
    ...
    setTimeout(function(){
      ...
      if(typeof attr !== typeof undefined && attr !== false && attr != ''){
        ...
        $.ajax({
              ...
              data: {id:GibberishAES.enc( $("input[name='tbl_id']").val(), toks), table:GibberishAES.enc( 'employee_pts', toks), employeeid:GibberishAES.enc( $("#employeeid_").val(), toks) , toks:toks},
              dataType: "JSON",
  ...
  function getDataRequestDetails(){
    $.ajax({
         ...
         data : {table:'employee_pts', employeeid: $("#employeeid_").val(), baseid: $("input[name='tbl_id']").val()},
         dataType: 'JSON',
         type : "POST",
...
$(".button_save_modal").unbind("click").click(function(){
  ...
  var fileName = filename = file = mime = ''; 
   ...
   if($("#myForm").valid()){
      ...
      if(cobj){
         ...
         // $(cobj).find("td:eq(3)").attr("relvenue", $("select[name='sm_venue']").val());  i;
 ...
 /*save/update data first*/
        ...
        if ("<?= $this->session->userdata("usertype") ?>" == "ADMIN" || "<?= $this->session->userdata("usertype") ?>" == "EMPLOYEE") {
         $("<div id='btn_pos' style='float: right;'><a class='btn btn-warning delete_pts' tbl_id='"+ tbl_id +"'><i class='glyphicon glyphicon-trash'></i></a></div>").click(function(){
         ...
         $("<div id='btn_pos' style='float: right;'><a class='btn btn-primary edit_pts' href='#modal-view' data-toggle='modal' tbl_id='"+ tbl_id +"' style='margin-right: 10px;'><i class='glyphicon glyphicon-edit'></i></a>").click(function(){
            addpts($(this).children(), tbl_id);
         }).appendTo($(mtd));
         }
         $(ntr).append($(mtd));
