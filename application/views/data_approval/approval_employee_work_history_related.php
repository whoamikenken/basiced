        ...
        <tbody>
            ...
            ?>
        		<tr id="<?= $value['id'] ?>" table="<?= $tbl ?>" title="<?= $title ?>">
                    ...
                    <td><?= $value['company']?></td>
                    <td><?= $value['remarks']?></td>
                    <td><?= (is_numeric($value['salary']) ? number_format($value['salary'], 2) : $value['salary'])?></td>
                    <td><?= $value['reason']?></td>
                    <td>
...
$("#familyTable").delegate(".edit_workhistoryrelated", "click", function(){
    addworkhistoryrelated($(this), $(this).attr("tbl_id"), $(this).attr("empname"), $(this).attr("employeeid"), $(this).attr("position"), $(this).attr("company"), $(this).attr("remarks"), $(this).attr("salary"), $(this).attr("reason"), $(this).attr("mime"), $(this).attr("content"), $(this).attr("dra_remarks") );
});
...
function addworkhistoryrelated(obj, tbl_id="", empname="", employeeid="", position="", company="", remarks="", salary="", reason="", mime="", content="", dra_remarks=""){
    $("#modal-view").find("h3[tag='title']").text(obj ? "Edit Work Experience of "+empname : "Add Work Experience");
     $("#button_save_modal").text("Save").attr("id", "savebtn").removeClass().addClass("btn btn-success button_save_modal_whr");
    $.ajax({
        ...
        success: function(msg){
            ...
            $.when($(modal_display).html(msg)).done(function(){ 
               if(obj){
                 ...
                 $(modal_display).find("input[name='wh_position']").val(position);
                 $(modal_display).find("input[name='wh_company']").val(company);
                 $(modal_display).find("input[name='wh_remarks']").val(remarks);
                 $(modal_display).find("input[name='wh_salary']").val(salary); 
                 $(modal_display).find("input[name='wh_reason']").val(reason);
