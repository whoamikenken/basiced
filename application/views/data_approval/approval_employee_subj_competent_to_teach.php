<div class="panel-body">
    ...
    <table class="table table-striped table-bordered table-hover" id="familyTable" width="100%">
        ...
        <tbody>
            ...
            list($value['filename'], $value['content'], $value['mime']) = $this->extensions->getEmployee201Files("employee_subj_competent_to_teach", $value['id']);
            ?>
        		<tr id="<?= $value['id'] ?>" table="<?= $tbl ?>" title="<?= $title ?>">
        			...
        			<td><?= $this->extras->getComTeachDesc($value['subj_id']); ?></td>
                    <td><?= $value['remarks']?></td>
                    <td>
                    ...
                    <td>
                        <div style="float: right">
                            <a class='btn btn-primary edit_sctt' href='#modal-view' data-toggle='modal' tbl_id = "<?= $value['id'] ?>" empname = "<?= $value['lname'].', '.$value['fname'].' '.$value['mname'] ?>"  employeeid="<?=$value['employeeid'] ?>" subj_id="<?=$value['subj_id'] ?>" remarks="<?=$value['remarks'] ?>" mime="<?= $value['mime']?>" content="<?= $value['content']?>"><i class='glyphicon glyphicon-edit'></i></a>&nbsp;
...
function loadRequestApprovalSSP(){
    ...
    table = $('#familyTable').DataTable({
        ...
        'createdRow': function( row, data, dataIndex ) {
              $(row).attr('table', $("#requestedData").val()).attr('title', $("#requestedData option:selected").text()).attr("id", data[7]);
        },
        'columnDefs': [
             {
                'targets': 0,
...
$("#familyTable").delegate(".edit_sctt", "click", function(){
    addSctt($(this), $(this).attr("tbl_id"), $(this).attr("empname"), $(this).attr("employeeid"), $(this).attr("subj_id"), $(this).attr("remarks") , $(this).attr("mime"), $(this).attr("content"), $(this).attr("dra_remarks") );
...
function addSctt(obj, tbl_id = "", empname = "", employeeid = "", subj_id="", remarks="" , mime="", content="", dra_remarks=""){
    $("#modal-view").find("h3[tag='title']").text(obj ? "Edit Subject Competent to Teach for "+empname : "Add Subject Competent to Teach");
    $("#button_save_modal").text("Save").attr("id", "savebtn").removeClass().addClass("btn btn-success button_save_modal_sctt");
    $.ajax({
        ...
        success: function(msg){
            ...
            $.when($(modal_display).html(msg)).done(function(){ 
               if(obj){
                 ...
                 $(modal_display).find("select[name='el_subj']").val(subj_id).trigger('chosen:updated');
