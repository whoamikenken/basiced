        ...
        <tbody>
           ...
           ?>
        		<tr id="<?= $value['id'] ?>" table="<?= $tbl ?>" title="<?= $title ?>">
                    ...
                    <td><?= $value['school']?></td>
                    <td><?= $value['honor'] ?></td>
                    <td><?= $value['year_grad'] ?></td>
                    <td>
                        <a class="btn btn-danger update_status tr_<?= $value['id'] ?>"><?= $value['status']?></a>
                    </td>
                    <td>
                        <div style="float: right">
                            <a class='btn btn-primary edit_community' href='#modal-view' data-toggle='modal' tbl_id = "<?= $value['id'] ?>" empname = "<?= $value['lname'].', '.$value['fname'].' '.$value['mname'] ?>"  employeeid="<?=$value['employeeid'] ?>" school="<?=$value['school'] ?>" honor="<?=$value['honor'] ?>" year_grad="<?=$value['year_grad'] ?>"  mime="<?= $value['mime']?>" content="<?= $value['content']?>"><i class='glyphicon glyphicon-edit'></i></a>&nbsp;
...
$("#familyTable").delegate(".edit_community", "click", function(){
    add_community($(this), $(this).attr("tbl_id"), $(this).attr("empname"), $(this).attr("employeeid"), $(this).attr("school"), $(this).attr("honor"),$(this).attr("year_grad"), $(this).attr("mime"), $(this).attr("content"), $(this).attr("dra_remarks") );
});
...
function add_community(obj, tbl_id="", empname="", employeeid="", school="", honor="", year_grad="", mime="", content="", dra_remarks=""){
    $("#modal-view").find("h3[tag='title']").text(obj ? "Edit Community Involvement of"+empname : "Add Community Involvement");
    $("#button_save_modal").text("Save").attr("id", "savebtn").removeClass().addClass("btn btn-success button_save_modal_community");  
    $.ajax({
        ...
        success: function(msg){
            ...
            $.when($(modal_display).html(msg)).done(function(){ 
               if(obj){
                 ...
                 $(modal_display).find("input[name='sm_school']").val(school);
                 // $(modal_display).find("select[name='sm_educational_level']").val(tdcur.find("td:eq(1)").attr("releduclevel")).trigger('chosen:updated');
                 $(modal_display).find("input[name='sm_year_grad']").val(year_grad);
                 $(modal_display).find("input[name='sm_honor']").val(honor);
