        		...
        		<tr id="<?= $value['id'] ?>" table="<?= $tbl ?>" title="<?= $title ?>">
                    ...
                    <td>
                        <div style="float: right">
                            <a class='btn btn-primary edit_pts_pdp2' href='#modal-view' data-toggle='modal' tbl_id = "<?= $value['id'] ?>" empname = "<?= $value['lname'].', '.$value['fname'].' '.$value['mname'] ?>"  employeeid="<?=$value['employeeid'] ?>" title_id="<?=$value['title_id'] ?>" datef="<?=$value['datef'] ?>" datet="<?=$value['datet'] ?>" organizer="<?=$value['organizer'] ?>" venue="<?=$value['venue'] ?>" location="<?=$value['location'] ?>" mime="<?= $value['mime']?>" content="<?= $value['content']?>"><i class='glyphicon glyphicon-edit'></i></a>&nbsp;
...
$("#familyTable").delegate(".edit_pts_pdp2", "click", function(){
    addpts_pdp2($(this), $(this).attr("tbl_id"), $(this).attr("empname"), $(this).attr("employeeid"), $(this).attr("title_id"), $(this).attr("datef"), $(this).attr("datet"), $(this).attr("organizer"), $(this).attr("venue"), $(this).attr("location"), $(this).attr("mime"), $(this).attr("content"), $(this).attr("dra_remarks") , $(this).attr("other_title")  );
});
...
function addpts_pdp2(obj, tbl_id="", empname="", employeeid="", title="", datef="", datet="", organizer="", venue="", location="", mime="", content="", dra_remarks="", other_title=""){
        $("#modal-view").find("h3[tag='title']").text(obj ? "Edit Program of "+empname : "Add Program");
        $("#button_save_modal").text("Save").attr("id", "savebtn").removeClass().addClass("btn btn-success button_save_modal_pts_pdp2");    
        $.ajax({
