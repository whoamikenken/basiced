        		...
        		<tr id="<?= $value['id'] ?>" table="<?= $tbl ?>" title="<?= $title ?>">
                    ...
                    <td>
                        <div style="float: right">
                            <a class='btn btn-primary edit_org' href='#modal-view' data-toggle='modal' tbl_id = "<?= $value['id'] ?>" empname = "<?= $value['lname'].', '.$value['fname'].' '.$value['mname'] ?>"  employeeid="<?=$value['employeeid'] ?>" name_org="<?=$value['name_org'] ?>" position="<?=$value['position'] ?>" datef="<?=$value['datef'] ?>" mime="<?= $value['mime']?>" content="<?= $value['content']?>"><i class='glyphicon glyphicon-edit'></i></a>&nbsp;
...
$("#familyTable").delegate(".edit_org", "click", function(){
    add_org($(this), $(this).attr("tbl_id"), $(this).attr("empname"), $(this).attr("employeeid"), $(this).attr("name_org"), $(this).attr("position"),$(this).attr("datef"), $(this).attr("mime"), $(this).attr("content"), $(this).attr("dra_remarks") );
});
...
function add_org(obj, tbl_id="", empname="", employeeid="", name_org="", position="", datef="", mime="", content="", dra_remarks=""){
    $("#modal-view").find("h3[tag='title']").text(obj ? "Edit Application and Civic Organization of "+empname : "Add Application and Civic Organization");
    $("#button_save_modal").text("Save").attr("id", "savebtn").removeClass().addClass("btn btn-success button_save_modal_org");
    $.ajax({
        ...
        success: function(msg){
            ...
            $.when($(modal_display).html(msg)).done(function(){ 
               if(obj){
                 ...
                 $(modal_display).find("input[name='sm_name_org']").val(name_org);
                 $(modal_display).find("input[name='sm_date']").val(datef);
                 $(modal_display).find("input[name='sm_position']").val(position);
