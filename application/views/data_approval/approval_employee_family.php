        		...
        		<tr id="<?= $value['id'] ?>" table="<?= $tbl ?>" title="<?= $title ?>" department="<?=$department?>" office="<?=$office?>" employeeid="<?=$employeeid?>"> 
                    ...
                    <td>
                        <div style="float: right">
                            <a class='btn btn-primary edit_family' href='#modal-view' data-toggle='modal' tbl_id = "<?= $value['id'] ?>" empname = "<?= $value['lname'].', '.$value['fname'].' '.$value['mname'] ?>" dob = "<?= $value['bdate']?>" reldata="<?= $value['relation']?>" relative = "<?= $value['name']?>" employeeid="<?=$value['employeeid'] ?>"><i class='glyphicon glyphicon-edit'></i></a>&nbsp;
...
$("#familyTable").delegate(".edit_family", "click", function(){
    addfamily($(this), $(this).attr("tbl_id"), $(this).attr("empname"), $(this).attr("dob"), $(this).attr("reldata"), $(this).attr("relative"), $(this).attr("employeeid"),$(this).attr("dra_remarks") );
});
...
function addfamily(obj, tbl_id = "", empname="",dob="",reldata="",relative="", employeeid="", dra_remarks=""){
     $("#familyTable").find("tr").each(function(){
       $(this).attr("iscurrent",0); 
     }) 
    $("#modal-view").find("h3[tag='title']").text(obj ? "Edit Family Member of "+empname : "Add Family Member");
    $("#button_save_modal").text("Save").attr("id", "savebtn").removeClass().addClass("btn btn-success button_save_modal_family");  
    $.ajax({
        ...
        success: function(msg){
            ...
            $.when($(modal_display).html(msg)).done(function(){
               if(obj){
                 var tdcur = $(obj).parent();
                 $(tdcur).attr("iscurrent",1);
                 $(modal_display).find("input[name='eb_name']").val(relative); 
                 $(modal_display).find("select[name='eb_relation']").val(reldata).trigger("chosen:updated");
                 $(modal_display).find("input[name='eb_dob']").val(dob);
