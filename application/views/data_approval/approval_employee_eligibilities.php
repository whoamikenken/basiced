    ...
    <table class="table table-striped table-bordered table-hover" id="familyTable" width="100%">
        ...
        <tbody>
        	<!-- <?php foreach ($tblData as $value):  
                    ...
                    list($value['filename'], $value['content'], $value['mime']) = $this->extensions->getEmployee201Files("employee_eligibilities", $value['id']);
        		...
        		<tr id="<?= $value['id'] ?>" table="<?= $tbl ?>" title="<?= $title ?>">
                    ...
                    <td>
                        <div style="float: right">
                            <a class='btn btn-primary edit_eligibility' href='#modal-view' data-toggle='modal' tbl_id = "<?= $value['id'] ?>" empname = "<?= $value['lname'].', '.$value['fname'].' '.$value['mname'] ?>"  employeeid="<?=$value['employeeid'] ?>" lnumber="<?=$value['license_number'] ?>" date_issued = "<?=$value['date_issued'] ?>" date_expired="<?=$value['date_expired'] ?>" description="<?=$value['description'] ?>" remarks="<?=$value['remarks'] ?>" mime="<?= $value['mime']?>" content="<?= $value['content']?>"><i class='glyphicon glyphicon-edit'></i></a>&nbsp;
...
$("#familyTable").delegate(".edit_eligibility", "click", function(){
    addeligibilities($(this), $(this).attr("tbl_id"), $(this).attr("empname"), $(this).attr("employeeid"), $(this).attr("lnumber"), $(this).attr("date_issued"), $(this).attr("date_expired"), $(this).attr("description"), $(this).attr("remarks"), $(this).attr("mime"), $(this).attr("content"), $(this).attr("dra_remarks"));
...
function addeligibilities(obj, tbl_id = "", empname = "", employeeid = "", lnumber="", date_issued="", date_expired="", description="", remarks="", mime="", content="", dra_remarks=""){
    $("#modal-view").find("h3[tag='title']").text(obj ? "Edit Eligibility of "+empname : "Add Eligibility");
    $("#button_save_modal").text("Save").attr("id", "savebtn").removeClass().addClass("btn btn-success button_save_modal_eligibility");  
    $.ajax({
        ...
        success: function(msg)
        {
            ...
            $.when($(modal_display).html(msg)).done(function(){ 
                if(obj)
                {
                    ...
                    $(modal_display).find("select[name='el_description']").val(description).trigger('chosen:updated');
                    $(modal_display).find("input[name='el_licenseNo']").val(lnumber);
                    $(modal_display).find("input[name='el_issuedDate']").val(date_issued); 
                    $(modal_display).find("input[name='el_expiryDate']").val(date_expired); 
                    $(modal_display).find("input[name='el_remarks']").val(remarks);
