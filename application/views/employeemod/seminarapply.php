            ...
            <div class="form_row">
                ...
                <div class="field">
                    <select class="chosen col-md-6" id="employeeids" name="employeeids" multiple="">
                        <?
                        # comment by justin (with e) for ica-hyperion 21152
                        ...
                        //}
                        ?>
                        <!-- new option list for ica-hyperion 21152 -->
                        <?
                            # list all employee under this department
                            $empID = $this->session->userdata('username');
                            $getEmployee = $this->extras->getAllEmployee($empID);
                            ksort($getEmployee);
                            foreach ($getEmployee as $key => $value) {?>
                                <option value="<?=$key?>"><?=$value?></option>
                        <?  } // end of foreach
                        ?>
                        <!-- end of new option list for ica-hyperion 21152 -->
                    </select>
                </div>
            </div>
...
$("#save").click(function(){
    ...
    if($("input[name='tfrom']").val() == "" || $("input[name='tto']").val() == ""){
        alert("Time in and out is required..");
        return false;
    }
    var tfrom = convertTimeToNumber($("input[name='tfrom']").val());
    var tto = convertTimeToNumber($("input[name='tto']").val());
    if(tfrom > tto || tto == tfrom || Number((tto - tfrom).toFixed(2)) == 0.02 || Number((tto - tfrom).toFixed(2)) == 0.01){
