...
$(document).ready(function(){
    ...
    if(depthead == 0){
        loadDeficiencyHistorys($("#userid").val(), "def");
    }else{
        loaddeficiencydata();
...
function loadDeficiencyHistorys(employeeid, def=''){
    $("#a_history").html("<img src='<?=base_url()?>images/loading.gif' />  Loading, Please Wait..");
    $.ajax({
        url      :   "<?=site_url("deficiency_/loadDeficiencyHistory")?>",
        type     :   "POST",
        data     :   {employeeid :  GibberishAES.enc(employeeid , toks), def: GibberishAES.enc(def , toks), toks:toks},
