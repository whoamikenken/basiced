        ...
        <div class="field">
            <table class="table table-hover table-bordered datatable" id="tableId">
                ...
                <tbody id="tbody-data">
                    ...
                    ?>
                        <tr>
                            <td style="<?=$style?>"><?=strtoupper(date('h:i: a',strtotime($row->timein)))?> - <?=strtoupper(date('h:i: a',strtotime($row->timeout)))?></td>
...
<?if($job != "lview"){?>
    <script>
        $(document).ready(function(){
           ...
           var filelink = "";
           if("<?=$leavetype?>" == "SL") filelink = '<td><a href="<?php echo site_url("leave_/viewUploadedFile?id=$aid")?>" target="_blank"><span class="pull-right" style="color: #6A1B9A;text-decoration: underline;"><b>View Medical Certificate<b></span></a></td>';

           $(".modal-header").html('<table width="100%"><tr><td rowspan="2" width="7%"><img src="<?=base_url()?>/images/school_logo.jpg" /></td><td><h4 class="modal-title"><strong><?=$this->extras->school_name()?></strong></h4></td></tr><tr><td><strong>Leave Application</strong></td>'+filelink+'</tr></table>');
        });
...
<?}else{?>
    <script>
        var filelink = "";
           if("<?=$leavetype?>" == "SL") filelink = '<td><a href="<?php echo site_url("leave_/viewUploadedFile?id=$base_id")?>" target="_blank"><span class="pull-right" style="color: #6A1B9A;text-decoration: underline;"><b>View Medical Certificate<b></span></a></td>';

        $(".modal-header").html('<table width="100%"><tr><td rowspan="2" width="7%"><img src="<?=base_url()?>/images/school_logo.jpg" /></td><td><h4 class="modal-title"><strong><?=$this->extras->school_name()?></strong></h4></td></tr><tr><td><strong>View Details</strong></td>'+filelink+'</tr></table>');
        $("#button_save_modal").hide();
    </script>
<?}?>