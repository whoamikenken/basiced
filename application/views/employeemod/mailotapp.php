...
// $cnoti = $this->employeemod->manageovertimenotif()->num_rows();
?>
<style type="text/css">
   .panel {
...
$("#searchlbtn").click(function(){
    ...
    loadOTAppList(dfrom, dto, category);
});

function loadOTAppList(datefrom, dateto, status, deptid='',office=''){
    var form_data = {
                        datefrom    : datefrom, 
                        dateto      : dateto,
                        deptid      : deptid,
    ...
    // $("#manageot").html("<img src='<?=base_url()?>images/loading.gif' />  Loading, Please Wait..");
    $.ajax({
       url      :   "<?=site_url("overtime_/getOTAppListToManage")?>",
       type     :   "POST",
       data     :   form_data,
