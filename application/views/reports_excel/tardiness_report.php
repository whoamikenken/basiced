...
$result = $this->attendance->giveAttendanceSummary($from_date, $to_date, $empid, $dept);
    ...
    $mainHeaders = array(
        ...
        array(
            'headerVal' => "TARDINESS/UNDERTIME FREQUENCY REPORT",
            'headerFont' => $fntNormalCenter,
            'doMerge' => true,
            'mergeCol' => $colsMerge,
            'mergeRow' => 0
...
if (count($result) > 0) {
    ...
    foreach ($result as $key => $datum) {
        ...
        foreach ($empSumm as $key => $summ2) {
            $tardy = ($this->time->hoursToMinutes($summ2["queTotalLateToday"]) > 0) 
                ? ($this->time->hoursToMinutes($summ2["queTotalLateToday"])) : "";

            if ($tardy != "") {
                array_push($arrLates, ($this->time->hoursToMinutes($summ2["queTotalLateToday"])));
            }

            $fntToWrite = ($tardy != "") ? $fntTardyCenter : $fntNormalCenter;

            // write tardy minutes to document
            $sheet->write($r,$c, $tardy, $fntToWrite);
            $c++;
        }

        // write totals
        $sheet->write($r,$c, count($arrLates), $fntNormalCenter);
        $c++;
        $sheet->write($r,$c, ($this->time->toHoursAndMinutes($arrLates)), $fntNormalCenter); 
        $c++;
