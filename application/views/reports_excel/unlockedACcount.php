...
//KENNEDY 
    ...
    $xls->send("Unlock Account History Report.xls");
    ...
    array_push($fields, array("UNLOCKED BY",1,20,1));
    array_push($fields, array("DATE UNLOCKED",1,20,1));
    ...
    $r++;
    $sheet->write(3,3,"Unlock Account History Report",$bigboldcenter);
    $r++;
    ...
    foreach ($data as $emp) 
    {
        ...
        $c++;
        $sheet->write($r,$c,iconv("UTF-8", "ISO-8859-1//IGNORE", $this->extensions->getEmployeeName($emp->employeeid)),$normalcenter);
        $c++;
        $sheet->write($r,$c,iconv("UTF-8", "ISO-8859-1//IGNORE",$this->extensions->getDeparmentDescriptionReport($this->extras->getemployeecol($emp->employeeid,"deptid"))),$normalcenter);
        $c++;
        $sheet->write($r,$c,iconv("UTF-8", "ISO-8859-1//IGNORE",$this->extensions->getPositionDescription($this->extras->getemployeecol($emp->employeeid,"positionid"))),$normalcenter);
        $c++;
        $sheet->write($r,$c,iconv("UTF-8", "ISO-8859-1//IGNORE",$this->extras->getAdminInfo($emp->updated_by)),$normalcenter);
        $c++;
