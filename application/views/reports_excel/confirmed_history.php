...
//KENNEDY 
    ...
    $xls->send("Attendance Confirmed History Report.xls");
    ...
    array_push($fields, array("STATUS",1,30,1));
    array_push($fields, array("DATE CONFIRMED",1,20,1));
    array_push($fields, array("CONFIRMED BY",1,20,1));
    ...
    $r++;
    $sheet->write(3,3,"Attendance Confirmed History",$bigboldcenter);
    $r++;
    $sheet->write(4,3,"As of ".date("F Y"),$normalcenter);
    $r++;
    $sheet->write(5,3,$payroll_cutoff,$normalcenter);
    $r++;
    ...
    foreach ($confirmed_list as $emp) 
    {
        ...
        $c++;
        $sheet->write($r,$c,iconv("UTF-8", "ISO-8859-1//IGNORE", $emp["dept"]),$normalcenter);
        $c++;
        $sheet->write($r,$c,iconv("UTF-8", "ISO-8859-1//IGNORE",$emp["status"]),$normalcenter);
        $c++;
        $sheet->write($r,$c,iconv("UTF-8", "ISO-8859-1//IGNORE", date("F d, Y H:i A", strtotime($emp["date_processed"]))),$normalcenter);
        $c++;
        $sheet->write($r,$c,iconv("UTF-8", "ISO-8859-1//IGNORE", $emp["confirmedby"]),$normalcenter);
        $r++;
        $c=0;
        $empcount++;
