...
// echo "<pre>"; print_r($emp_list); die;
    ...
    if($sd_filter == "detailed"):
        // header
        $row = $col = 0;
        if($teachingtype == "teaching" || $teachingtype == ""){
            ...
            // writeBorder($sheet, count($income_header + $income_header), 0, $row, $coltitle);
            // $row = writeText($sheet, "  INCOME AND DEDUCTION", $coltitle, $row, $col, 0, (count($income_header) - 1));

            $income_header_upper = array();
            $income_header_upper[] = array("", 20, $coltitle);
            $income_header_upper[] = array("", 20, $coltitle);
            $income_header_upper[] = array("", 20, $coltitle);
            $start_income = (round((9+(count($inc_income['deminimissList']))+(count($inc_income['noDeminimissList']))+(count($inc_adjustment))) / 2) - 1);
            $start_deduction = (round((3+(count($inc_fixed_deduc))+(count($inc_deduction))+(count($inc_loan))) / 2) - 1);
            for ($i=0; $i < (9+(count($inc_income['deminimissList']))+(count($inc_income['noDeminimissList']))+(count($inc_adjustment))); $i++) { 
                if($i == $start_income)  $income_header_upper[] = array("INCOME", 20, $coltitle);
                $income_header_upper[] = array("", 20, $coltitle);
            }

            for ($i=0; $i < (3+(count($inc_fixed_deduc))+(count($inc_deduction))+(count($inc_loan))); $i++) { 
                if($i == $start_deduction)  $income_header_upper[] = array("DEDUCTION", 20, $coltitle);
                $income_header_upper[] = array("", 20, $coltitle);
            }
            // $income_header_upper[] = array("", 20, $coltitle);
            // $sheet->setMerge($r_, $c_, $r_, (9+(count($inc_income['deminimissList']))+(count($inc_income['noDeminimissList'])))); 
            $row = writeTable($sheet, array($income_header_upper), 0, $row+1);
            ...
            $old_deptid = '';
            $employee_count = 0;
            if($sort == 'name' && !isset($teaching)){
                $teaching['emp_list'] = $emp_list;
            }
        ...
        if($teachingtype == "nonteaching"  || $teachingtype == ""){
            if($teachingtype == "nonteaching"){
                ...
                $bitmap = "images/school_logo.bmp";
                $sheet->insertBitmap( ($row - 5) , 4 , $bitmap , 80 , 0 , .15 ,.20 );
            ...
            if($sort == 'name' && !isset($nonteaching) && $teachingtype != ""){
                $nonteaching['emp_list'] = $emp_list;
            }else{
                $nonteaching['emp_list'] = array();
            }
    ...
    # ======================================================== SUMMARY  ========================================================
        ...
        $bitmap = "images/school_logo.bmp";
       $sheet->insertBitmap( 1 , 2 , $bitmap , 80 , 0 , .15 ,.20 );
        ...
        foreach ($summary as $sort_key => $info) {
            ...
            $content_table[] = array($config['campus'][$sort_key], 50, $normalCenterBorder);
            $content_table[] = array($info["income"]["salary"], 20, $normalRightBorder);
            $content_table[] = array($info["income"]["tardy"], 20, $normalRightBorder);
            $content_table[] = array($info["income"]["absent"], 20, $normalRightBorder);
            $content_table[] = array($info["income"]["basic_pay"], 20, $normalRightBorder);
