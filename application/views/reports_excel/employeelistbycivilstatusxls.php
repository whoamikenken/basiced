...
//KENNEDY 
    ...
    $xls->send("Employee List By Civil Status.xls");
    ...
    array_push($fields, array("EMPLOYEE NAME",1,40,1));
    ...
    $sheet->insertBitmap( $r , 1 , $bitmap , $hr , 10 , .10 ,.20 );
    $r++;$c++;
    $sheet->write(1,2,$SCHOOL_NAME,$boldcenter);
    $r++;
    $sheet->write(2,2,$SCHOOL_CAPTION,$boldcenter);
    $r++;
    $sheet->write(3,2,strtoupper($reportTitle)." REPORT",$bigboldcenter);
    $r++;
    $sheet->write(4,2,"As of ".date("F Y"),$normalcenter);
    ...
    foreach ($cdata as $emp) 
    {
        if($CS != $emp->civil_status)
        {
            ...
            $CS = $emp->civil_status;
            if ($emp->civil_status){
                $display = $this->reports->getCivilStatus($emp->civil_status);
                $display = $display[0]->description;
            }
            else $display = "NO CIVIL STATUS";
                
            $sheet->write($r,$c,iconv("UTF-8", "ISO-8859-1//IGNORE",$display),$colOffice);
            $sheet->setMerge($r,$c,$r,$c+2);
            $r++;
            $c=0;
        }
