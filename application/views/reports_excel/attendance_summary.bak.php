 ...
 */
    ...
    $sql = $this->db->query("SELECT mtable.employeeid,mtable.employeecode,CONCAT(mtable.lname,', ',mtable.fname,' ',mtable.mname) AS Fullname,mtable.deptid,c.description AS department,DATE(mtable.dte) AS logdate,
                            ...
                            LEFT JOIN 
                            (SELECT a.userid,a.timein,a.timeout,a.mac_add_in,a.mac_add_out FROM timesheet a 
                               UNION 
             ...
             /**                
    $sql = $this->db->query("SELECT a.employeeid,CONCAT(a.LName,', ',a.FName,' ',a.MName) AS Fullname,a.deptid,c.description as department,DATE(e.dte) as logdate,
                                IFNULL((SELECT b2.logtime FROM timesheet_trail b2 
                                 WHERE DATE(b2.logtime)=DATE(e.dte) AND b2.userid=a.employeeid AND b2.log_type='IN' ORDER BY b2.logtime LIMIT 1),'') AS log_IN,
                                IFNULL((SELECT b2.logtime FROM timesheet_trail b2 
                                 WHERE DATE(b2.logtime)=DATE(e.dte) AND b2.userid=a.employeeid AND b2.log_type='OUT' ORDER BY b2.logtime DESC LIMIT 1),'') AS log_OUT
                             FROM employee a 
                             LEFT JOIN (SELECT '".date("Y-m-d",strtotime($datesetfrom))."' + INTERVAL a1 + b1 + c1 DAY dte
                                        FROM
                                         (SELECT 0 a1 UNION SELECT 1 UNION SELECT 2 UNION SELECT 3
                                            UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7
                                            UNION SELECT 8 UNION SELECT 9 ) d,
                                         (SELECT 0 b1 UNION SELECT 10 UNION SELECT 20 UNION SELECT 30 UNION SELECT 40 UNION SELECT 60 UNION SELECT 70 UNION SELECT 80 UNION SELECT 90) m , 
                                         (SELECT 0 c1 UNION SELECT 100 UNION SELECT 200 UNION SELECT 300 UNION SELECT 400 UNION SELECT 600 UNION SELECT 700 UNION SELECT 800 UNION SELECT 900) h
                                        WHERE '".date("Y-m-d",strtotime($datesetfrom))."' + INTERVAL a1 + b1 + c1 DAY  <=  '".date("Y-m-d",strtotime($datesetto))."'
                                        ORDER BY a1 + b1 + c1) e ON e.dte<>''{$condition_date}
                             LEFT JOIN code_office c ON c.code=a.deptid
                             LEFT JOIN timesheet_trail b ON a.employeeid=b.userid AND DATE(b.logtime)=e.dte
                             WHERE a.employeeid<>''{$condition}
                             GROUP BY a.employeeid,DATE(e.dte) ORDER BY department,Fullname,DATE(e.dte)")->result_array();
                             */
