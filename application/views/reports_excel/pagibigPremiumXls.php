<?php
    ...
    $xls->send("PAGIBIGPREMIUM".date("mY",strtotime($dfrom)).".xls");
    ...
    $r++;
    $sheet->write(2,0,"PagIBIG Branch: ",$boldleft);   $sheet->writeString(2,1,'03',$normalleft);
    $r++;
    ...
    $r++;
    $sheet->write(6,0,"Pay Type:",$boldleft);       $sheet->write(6,1,'MC',$normalleft); $sheet->write(6,2,'(MC/CL/ST)',$normalleft);
    $r++;
    ...
    $r = writeTable($sheet,array(0 => $tbl_header), 13, 0, $boldleft, $table_size);
        ...
        $table_line = array("'------------------------------------------","'------------------------------------------","'------------------------------------------","'------------------------------------------","'------------------------------------------","'------------------------------------------","'------------------------------------------","'------------------------------------------","'------------------------------------------");
    $r = writeTable($sheet,array(0 => $table_line), 14, 0, $boldleft, $table_size);

    /*$r = 15;
    $c = 0;*/
    $query = $this->db->query("SELECT * FROM hdmf_deduction");
    foreach ($query->result() as $value) {
        $employeeCont = $value->emp_ee;
        $employeerCont = $value->emp_er;
    }
...
$empdata = $this->payroll->getpagibigLoanExcel($empid,$schedule,$quarter,$dfrom,$dto,$dept);    
    ...
    foreach ($empdata->result() as $info) {
        ...
        ($info->emp_tin == "NULL" || $info->emp_tin == NULL) ? $emptin = "" : $emptin = $info->emp_tin;
            ...
            $r = writeTable($sheet,array(0 => $tbl_content), $r, 0, $normalleft, $table_size);          
