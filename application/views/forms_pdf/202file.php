...
function getEdu($eid,$level,$data){
    $queue = mysql_query("SELECT $data FROM employee_education WHERE employeeid='$eid' AND FIND_IN_SET(educational_level,'$level')");
    $rs = mysql_fetch_array($queue);
    return $rs[$data];
}

function getOtherSkill($eid){
    $return = "";
    $query = mysql_query("SELECT skills FROM employee_skills WHERE employeeid='$eid'");
...
$query = mysql_query("SELECT CONCAT(FName,' ',SUBSTR(MName,1,1),'. ',LName) AS fullname, b.description AS religion,a.prc,c.description AS civilstatus, d.description AS citizenship, IF(gender='F','FEMALE','MALE') as gender, DATE_FORMAT(bdate,'%M %d, %Y') as bdate,
                        	emp_sss, emp_tin, emp_philhealth, emp_pagibig, cityaddr, mobile, citytelno, email, spouse_name, passport
                        FROM employee a
...
// Work History
$qhis = mysql_query("SELECT POSITION, company, DATEDIFF(date_to,date_from) AS days, (DATEDIFF(date_to,date_from)/365) AS years from employee_work_history WHERE employeeid='$eid'");
$qhiscount = mysql_num_rows($qhis);

// Position History
$phis = mysql_query("SELECT description,dateposition,assignment,remarks FROM employee_position_history a INNER JOIN code_position b ON b.positionid = a.position WHERE employee='$eid';");
$phiscount = mysql_num_rows($phis);

$iquery = mysql_query("SELECT * FROM elfinder_file where title='$eid'");
$irs = mysql_num_rows($iquery);
if($irs > 0)
    $img = "<img src='".site_url('forms/loadForm')."?form=imgview&eid=$eid' width='25%' height='30%' style='border: 1px solid black'/>";
else
    $img = "<img src='".base_url()."images/no_image.gif' width='25%' height='30%'/>";
...
<div class='fcontainer' >
    <div style='color: black; margin-left: 20px; margin-top: 5px; font-size: 12px; font-family: Amazone BT, times, serif; '><b>ID NO. $eid</b></div>
    ...
    <div class='content'>
        <table width='100%' style='border-collapse: collapse; table-layout: fixed;'>
            <tr>
                ...
                <td class='regu' width='10%'>&nbsp;</td>
                <td class='regu' width='25%'>&nbsp;</td>
                <td class='regu' width='30%' rowspan='12' style='text-align: center;'>$img</td>
            </tr>
            <tr>
        ...
        <table width='100%' style='border-collapse: collapse; table-layout: fixed; margin-top: 5px;'>
            ...
            <tr>
                ...
                <td class='reguC bordercontent' width='20%'>Year Graduated</td>
            ...
            <tr>
                ...
                <td class='regu bordercontent'>".getEdu($eid,'ELEMENTARY','school')."</td>
                <td class='regu bordercontent' style='text-align: center;'>".getEdu($eid,'ELEMENTARY','year_graduated')."</td>
                <td class='regu borderlast'></td>
            </tr>
            <tr>
                <td class='regu bordercontent' height='50'>High School</td>
                <td class='regu bordercontent'>".getEdu($eid,'HIGH SCHOOL','school')."</td>
                <td class='regu bordercontent' style='text-align: center;'>".getEdu($eid,'HIGH SCHOOL','year_graduated')."</td>
                <td class='regu borderlast'></td>
            </tr>
            <tr>
                <td class='regu bordercontent' height='50'>College</td>
                <td class='regu bordercontent'>".getEdu($eid,'COLLEGE','school')."</td>
                <td class='regu bordercontent' style='text-align: center;'>".getEdu($eid,'COLLEGE','year_graduated')."</td>
                <td class='regu borderlast'></td>
            </tr>
            <tr>
                <td class='regu bordercontent' height='50'>Graduate Studies</td>
                <td class='regu bordercontent'>".getEdu($eid,'DOCTORAL,MASTERAL','school')."</td>
                <td class='regu bordercontent' style='text-align: center;'>".getEdu($eid,'DOCTORAL,MASTERAL','year_graduated')."</td>
                <td class='regu borderlast'></td>
            </tr>
        </table>
        <table width='100%' style='border-collapse: collapse; table-layout: fixed; margin-top: 5px;'>
            <tr>
                ...
                <td class='regu line' width='30%'>".getEdu($eid,'COLLEGE','major')."</td>
                <td class='regu' width='15%'>&nbsp;</td>
                <td class='regu' width='10%' style='text-align: right;'>MINOR:</td>
                <td class='regu line' width='30%'>".getEdu($eid,'COLLEGE','minor')."</td>
            </tr>
            <tr>
                <td class='regu'>OTHER SKILL:</td>
                <td class='regu line' colspan='4'>".getOtherSkill($eid)."</td>
            </tr>
            <tr>
        ...
        <table width='100%' style='border-collapse: collapse; table-layout: fixed;'>
            ...
            if($qhiscount > 0){
            while($limit != 0){
            while($rq = mysql_fetch_array($qhis)){
$info .=   "
                <tr>
                    <td class='regu bordercontent' style='text-align: center;'>{$rq['POSITION']}</td>
                    <td class='regu bordercontent' style='text-align: center;'>{$rq['company']}</td>
                    <td class='regu borderlast' style='text-align: center;'>".number_format($rq['years'])." year ({$rq['days']} days)</td>
                </tr>";
            $limit--;}
$info .=   "
            <tr>
...
$info .=   "
        ...
        <table width='100%' style='border-collapse: collapse; table-layout: fixed;'>
            <tr style='background-color: #916E34'>
                <td class='reguC borderheader bordercontent' width='30%' style='color: white;'>POSITION</td>
                <td class='reguC borderheader bordercontent' width='20%' style='color: white;'>SCHOOL YEAR</td>
                <td class='reguC borderheader bordercontent' width='35%' style='color: white;'>ASSIGNMENT</td>
                <td class='reguC borderlast bordertop' width='15%' style='color: white;'>Remarks</td>
            </tr>";
        if($phiscount > 0){
            while($limit != 0){
            while($pdata = mysql_fetch_array($phis)){
$info .= "
            <tr>
                <td class='regu bordercontent' style='text-align: center;'>{$pdata['description']}</td>
                <td class='regu bordercontent' style='text-align: center;'>".date('F d, Y',strtotime($pdata['dateposition']))."</td>
                <td class='regu bordercontent'>{$pdata['assignment']}</td>
                <td class='regu borderlast '>{$pdata['remarks']}</td>
            </tr>
         ";
            $limit--;}
$info .=   "
