...
$data = $this->reports->SSSContributionPerMY($pfrom,$pto,$pyearfrom,$pyearto,$employeeid,$sort_by);
$datas = $this->reports->SSSContributionPerMY($pfrom,$pto,$pyearfrom,$pyearto,$employeeid,$sort_by);
	foreach ($data as $k => $val) {
		$cutoffstart = $val->cutoffstart;
		$cutoffend  = $val->cutoffend;
		$fixeddeduc = $val->fixeddeduc;
		...
		if($currentMonth == $cutoffMonth){
			...
			$ec = $ec + $currentEc;
			$ssstotal = $ee + $er + $ec;
			$emplist[$val->employeeid][$cutoffYear][$cutoffMonth] = array("SSS" =>$sss,'baseid'=>$baseid,"OR"=>$or,"Datepaid"=>$datepaid,"EE"=>$ee,"EC"=>$ec,"ER"=>$er,"SSSTotal"=>$ssstotal);
		}else{
			$emplist[$val->employeeid][$cutoffYear][$cutoffMonth] = array("SSS" =>$sss,'baseid'=>$baseid,"OR"=>$or,"Datepaid"=>$datepaid,"EE"=>$ee,"EC"=>$ec,"ER"=>$er,"SSSTotal"=>$ssstotal);
		}
		$emplist[$val->employeeid]['info'] = array("fullname"=>$fullname,'sssno'=>$sssnumber);
...
<htmlpageheader name='Header'>
    <div>
        <table width='60%'  >
            ...
            <tr>
                <td valign='middle' style='padding: 0;text-align: center; margin-left:100px;'><span style='font-size: 13px; font-family:times new roman;' width='55%'><strong>Certificate of SSS Contribution</strong></span></td>
            </tr>
            <tr>
...
$fieldname = array("YEAR","MONTH","AMOUNT SSS","AMOUNT EC","OR# / SBR#","DATE PAID");
...
foreach ($emplist as $employeeid => $field) {
	 ...
	 $empid = $employeeid;
		...
		$info .="
				<table width='100%' class='maintable' border=1>
					<thead>
					<tr class='noBorders'>
						<td border='0' colspan='6'><div class='dataBold'>DATE: ".date('F d, Y')."</div></td>
					</tr>
					<tr class='noBorders'>
						<td colspan='6'><div class='dataBold'><span id='sss'>SSS Contributions - Summary of Actual Premiums</span></div></td>
					</tr>
					<tr class='noBorders' >
						<td width='15%' id='fontfam'>Company Name </td><td width='35%' colspan='2' id='fontfam'><b>Pinnacle Technologies Inc.</b></td><td width='15%' id='fontfam'>&emsp;&emsp;&emsp;&emsp;&emsp;Employee&nbsp;Name </td><td width='35%' colspan='2' id='fontfam' color='black'><b>&emsp;&emsp;".$field['info']["fullname"]."</b></td>
					</tr>
					<tr class='noBorders'>
						<td id='fontfam'>Employer ID No. </td><td width='35%' colspan='2' id='fontfam'><b>37-0077400-7</b></td><td id='fontfam'>&emsp;&emsp;&emsp;&emsp;&emsp;SSS&nbsp;ID&nbsp;No. </td><td width='35%' colspan='2' color='black' id='fontfam'><b>&emsp;&emsp;".$field['info']['sssno']."</b></td>
					</tr>
					<tr class='noBorders'>
						<td colspan='3'></td><td id='fontfam' >&emsp;&emsp;&emsp;&emsp;&emsp;Period&nbsp;Covered </td><td width='35%' colspan='2' id='fontfam'><b>&emsp;&emsp;".$arrayMonths[$pfrom]." ".$pyearfrom." - ".$arrayMonths[$pto]." ".$pyearto."</b></td>
					</tr>
					<tr class='noBorders'>
						<td colspan='6' id='fontfam'><b>Monthly Premium</b></td>
					</tr>";
		$info  .="</thead></table><table width='100%' class='maintable' border=1><thead><tr bgcolor='#000000'>";
				$widthper = count($fieldname);
				$widthper = 100 / $widthper;
					foreach ($fieldname as $key =>$title) 
					{

						$info .= "<td color='yellow' align='center' id='fontfam' width='".$widthper."%'><b>".$title."</b></td>";
					};	
			$info  .="</tr></thead>";
			krsort($genYearMonth);
						...
						if (isset($monthcode[$days])) {
								...
								if ($sameyear != $year) {
									$info .="<tbody>
										...
										if (isset($field[$year][$days]["SSS"])) {

											$info .="<td style='text-align:right' id='fontfam'>".number_format($field[$year][$days]["SSSTotal"],2)."</td>
										...
										{
											$info .="<td style='text-align:right' id='fontfam'> 0.00</td ><td style='text-align:right' id='fontfam'>0.00</td>";
										}
										$info .="
								...
								else
								{
									$info .= "<tr>
											<td></td>
											<td>".$description."</td>";
									if (isset($emplist[$empid][$year][$days]["SSS"])) {
										$info .="<td style='text-align:right' id='fontfam'>".number_format($field[$year][$days]["SSSTotal"],2)."</td>
											<td style='text-align:right' id='fontfam'>".number_format($field[$year][$days]["EC"],2)."</td>";
										// $info .="<td style='text-align:right' id='fontfam'>".number_format($this->reports->getSSSContribution($field[$year][$days]['baseid'],$field[$year][$days]["SSS"],'totalsss'),2)."</td>
										// 		<td style='text-align:right' id='fontfam'>".number_format($this->reports->getSSSContribution($field[$year][$days]['baseid'],$field[$year][$days]["SSS"],'ec'),2)."</td>";
	...
	$info .= "
			...
			<tr class='noBorders' >
				<td colspan='6'><div  class='font'>This cerfication is issued upon request of the above-mentioned name for SSS reference.</div></td>
			</tr>
			<tr class='noBorders' >
				<td>&nbsp;</td>
			</tr>
			...
			<tr class='noBorders'>
				<td colspan='4'></td>
				<td style='white-space: nowrap;><div  class='approverposition' style='white-space: nowrap;>".$this->extensions->getPositionDesc($this->extensions->getEmployeePositionId($certifiedcorrect))."</div></td>
			</tr>
	</table>
