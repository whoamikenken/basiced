...
function getDataamt($eid,$sched,$quarter,$sdate,$edate,$dept,$col,$ttle){
    $amt = "";
    $whereClause = "";
    if($eid)    $whereClause .= " AND employeeid='$eid'";
    $query = mysql_query("SELECT $col FROM payroll_computed_table WHERE schedule='$sched' AND quarter='$quarter' AND cutoffstart='$sdate' AND cutoffend='$edate' $whereClause");
    ...
    if($data){
        ...
        for($x = 0; $x < count($iex); $x++){
            $singleex = explode('=',$iex[$x]); 
            if($singleex[0] == $ttle)   $amt     = $singleex[1];
        }
    }
    return $amt;
}

function getincattr($col,$tbl,$amt){
    $wC = " WHERE emp_ee='$amt'";
    $query = mysql_query("SELECT $col FROM $tbl $wC");
    $rs = mysql_fetch_array($query);
    if(mysql_num_rows($query) > 0) return $rs[$col];
    else                           return "";
}
...
function getTotali($eid,$sched,$quarter,$sdate,$edate,$dept,$ttle,$col,$tbl){
    $amt = $iamt = array();
    $whereClause = "";
    $yr = date('Y',strtotime($sdate));
    if($eid)    $whereClause .= " AND employeeid='$eid'";
    ...
    $query = mysql_query("SELECT fixeddeduc FROM payroll_computed_table WHERE schedule='$sched' AND SUBSTR(cutoffstart,1,4) = '$yr' AND cutoffend <= '$edate' $whereClause AND fixeddeduc <> ''"); 
    while($rs = mysql_fetch_array($query)){
        $data = $rs['fixeddeduc'];
        if($data){
            $iex = explode('/',$data);
            for($x = 0; $x < count($iex); $x++){
                ...
                if($singleex[0] == $ttle){
                  #  if($singleex[1] > 0){
                        if($col){
                            $qry = mysql_query("SELECT $col FROM $tbl WHERE emp_ee='$singleex[1]'");
                            $rsi = mysql_fetch_array($qry);
    ...
    if($col)    return array_sum($iamt);
    else        return array_sum($amt);
}
