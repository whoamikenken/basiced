...
function getEmpDesc($eid = "",$dept = ""){
    $return = "";
    if($eid)  $whereClause = " WHERE a.employeeid='$eid'";
    if($dept) $whereClause = " WHERE a.deptid='$dept'";
    $query = mysql_query("SELECT b.description FROM employee a INNER JOIN code_office b ON a.deptid = b.code $whereClause");
...
function getData($eid,$sched,$quarter,$sdate,$edate,$dept,$col,$th = false){
    ...
    $whereClause = $limit = "";
    if($eid)    $whereClause .= " AND a.employeeid='$eid'";
    if($th && $dept)   $whereClause .= " AND deptid='$dept'";
    if($th) $limit = " LIMIT 1";
    $query = mysql_query("SELECT $col FROM payroll_computed_table a INNER JOIN employee b ON a.employeeid = b.employeeid WHERE schedule='$sched' AND quarter='$quarter' AND cutoffstart='$sdate' AND cutoffend='$edate' $whereClause $limit");
