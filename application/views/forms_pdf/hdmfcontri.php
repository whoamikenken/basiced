...
$data = $this->reports->hdmfContributionPerMY($pfrom,$pto,$pyearfrom,$pyearto,$employeeid, $sort_by);
$datas = $this->reports->hdmfContributionPerMY($pfrom,$pto,$pyearfrom,$pyearto,$employeeid, $sort_by);
$currentMonth = $currentEe = $currentEc = $currentEr = '~~';
	foreach ($data as $k => $v) {
		...
		if($currentMonth == $cutoffMonth){
			...
			$ec = $ec + $currentEc;
			$pagibigTotal = $ee + $er + $ec;
			$emplist[$v->employeeid][$cutoffYear][$cutoffMonth] = array("PAGIBIG" =>$hdmf,'baseid'=>$baseid,"OR"=>$or,"Datepaid"=>$datepaid,"EE"=>$ee,"EC"=>$ec,"ER"=>$er,"PAGIBIGTotal"=>$pagibigTotal);
		}else{
			$emplist[$v->employeeid][$cutoffYear][$cutoffMonth] = array("PAGIBIG" =>$hdmf,'baseid'=>$baseid,"OR"=>$or,"Datepaid"=>$datepaid,"EE"=>$ee,"EC"=>$ec,"ER"=>$er,"PAGIBIGTotal"=>$pagibigTotal);
		}
		
		$emplist[$v->employeeid]['info'] = array("fullname"=>$fullname,'pagibigno'=>$hdmfnumber);
...
<htmlpageheader name='Header'>
    <div>
        <table width='60%'  >
            ...
            <tr>
                <td valign='middle' style='padding: 0;text-align: center; margin-left:100px;'><span style='font-size: 13px;' width='55%'><strong>Certificate of PAG-IBIG Fund Contribution</strong></span></td>
            </tr>
            <tr>
...
foreach ($emplist as $employeeid => $field) {
	...
	$empFullname = $field["fullname"];
     $empHDMFno = $field["pagibigno"];
     $empid = $employeeid;
     $sameyear = "";

	$info .="<div class='dataBold'>DATE: ".date('F d, Y')."</div>
	<div class='dataBold'><span id='hdmf'>PAG-IBIG Fund Contributions - Summary of Actual Premiums</span></div>
			...
			<table width='100%' class='maintable' border=1>
				...
				<tr class='noBorders'>
					<td>Employer ID No. </td><td width='35%' colspan='2'><b>37-0077400-7</b></td><td>&emsp;&emsp;&emsp;&emsp;&emsp;PAG-IBIG&nbsp;ID&nbsp;No. </td><td width='35%' colspan='2'><b>&emsp;&emsp;".$field['info']["pagibigno"]."</b></td>
				</tr>
				<tr class='noBorders'>
						...
						$datepaidFormat = isset($field[$year][$days]["Datepaid"])?date('m-d-Y',strtotime($field[$year][$days]["Datepaid"])):"";
								if ($sameyear != $year) {
									$info .="
										...
										if (isset($field[$year][$days]["PAGIBIG"])) {
// $info .="<td style='text-align:right' >".number_format($this->reports->getpagibigContribution($field[$year][$days]['baseid'],$field[$year][$days]["PAGIBIG"],'totalpagibig'),2)."</td>
// <td style='text-align:right' >".number_format($this->reports->getpagibigContribution($field[$year][$days]['baseid'],$field[$year][$days]["PAGIBIG"],'er'),2)."</td>";
											$info .="<td style='text-align:right'>".number_format($field[$year][$days]["EE"],2)."</td>
...
// <td style='text-align:right' >".number_format($this->reports->getpagibigContribution($field[$year][$days]['baseid'],$field[$year][$days]["PAGIBIG"],'er'),2)."</td>";
		...
		$info .= "
			...
			<tr class='noBorders' >
				<td colspan='6'><div  class='font'>This cerfication is issued upon request of the above-mentioned name for PAG-IBIG reference.</div></td>
			</tr>
			<tr class='noBorders' >
				<td>&nbsp;</td>
			</tr>
