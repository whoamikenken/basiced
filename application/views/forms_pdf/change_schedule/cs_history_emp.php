            ...
            <tr <?=(!$row->isread ? " style='background: #B4CDC6'" : "")?>>
                <td >
                  <?php if ($row->status == 'PENDING' && ($row->dstatus == 'PENDING' || $isHead) && $row->cstatus == 'PENDING' && $row->cstatus == 'PENDING' && $row->hrstatus == 'PENDING' && $row->cpstatus == 'PENDING' && $row->fdstatus == 'PENDING' && $row->bostatus == 'PENDING' && $row->pstatus == 'PENDING' && $row->upstatus == 'PENDING' ): ?>
                    <div class="btn-group">
                  <a class="btn btn-info editbtn" href="#modal-view" data-toggle="modal"   data-target="#myModalatts" idkey="<?=$row->csid?>" base_id="<?=$row->base_id?>"><i class="glyphicon glyphicon-edit"></i></a>
...
$("#seminarh_length").append("<span style='margin-left: 45px;'>Status : <select id='seminarstatus' style='margin-bottom: 2px;'><?=$this->extras->showCategoryopt($status)?></select></div>");
