...
<form>
    <div class="col-md-12">
        ...
        <div class="form-group">
            <label for="serial_number">Student</label>
        ...
        <div class="form-group">
            <label for="serial_number">Card</label>
            <input class="form-control" id="card" name="card" type="text" value="<?= isset($card) ? $card : ""?>"/>
        </div>
...
$("#button_save_modal").unbind('click').bind('click', function(event) {
    ...
    if(serial_number != "" && name != "" && employeeid != ""){
        $.ajax({
        type: "POST",
        url: "<?= site_url('facial_/savePersonStudent')?>",
        data: fd,
        dataType:"JSON",
...
function loadEmp(emp){
    $.ajax({
        url: "<?= site_url('facial_/loadStudent') ?>",
        type: "POST",
        data: {
            emp: emp
        },
