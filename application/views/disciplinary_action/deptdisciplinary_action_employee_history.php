<?
	...
	$excessiveDeptAbsenteismCount = $CI->disciplinary_action->empWithExcessiveAbsenteism(true);
...
<div id="content" style="margin-right: 10px;">
	<div style='float:right;padding-right:1%;padding-top:1%;font-size:10px'>
		<span style="font-size: 15px;">Year</span> &nbsp; <select class="chosen" id="pyear" style="width: 100px;"><?=$this->payrolloptions->periodyear();?></select>&nbsp;
		...
		|<a href='#' style="font-weight:bold;text-decoration: underline;font-size:1.5em;margin-right: 10px;" id="excessiveAbsenteism" class='list' type='Absenteism'>List of Employee with Excessive Absenteism
			...
			<div class="col-md-12">
				<div class="panel animated fadeIn delay-1s">
					<div class="well-header">
						<div class="panel-heading" style="background-color: #0072c6; color: black"><h4><b>List of Employees</b></h4></div>
					</div>
					<div class="panel-body">
						<!-- <a class="btn batchaddbtnsanction" href="#modal-view" data-toggle="modal" style='margin-bottom:1%'><i class="icon-plus-sign"></i><span class="" style="font-family: Tahoma;"> Add by Batch </span> -->
						<!-- </a> -->
						<table class="table table-striped table-bordered table-hover datatable" id='table'>
							...
							<?php
    						$emplist = $CI->disciplinary_action->getEmployeeByDept($username);
    						$deptId = isset($emplist[0]->deptid) ? $emplist[0]->deptid : '';
    						$deptDesc = $CI->extras->getDeptDesc($deptId);
								foreach($emplist as $row){
									foreach ($row as $key => $value) $row->$key = Globals::_e($value);

								?>
								<tr style="cursor: pointer;" employeeid="<?=$row->employeeid?>" positionid="<?=$row->positionid?>"  deptid="<?=$row->deptid?>"  fname="<?=$row->fname?>">
...
function loadNotifCount(year){
	$.ajax({
        ...
        type: "POST",
        data: {year: GibberishAES.enc(year , toks), department: GibberishAES.enc( department, toks), toks:toks},
        dataType: "JSON",
...
$("#employeelist tr").click(function(){
   if($(this).attr("employeeid")){
       var form_data = {
        ...
        view:  GibberishAES.enc("disciplinary_action/add_emp_disciplinary_action", toks),
        toks:toks
       }; 
       $.ajax({
			...
			success: function(msg){
				$("#content").html(msg);
				$("a[name='backlist']").attr("reload", "yes");
			}
       });
   }
});  
