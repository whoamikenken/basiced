...
?>
<!-- add by justin (with e) for ica-hyperion 21194 -->
<style>
    .modal{
        width: 700px;
    }
...
<div class="form_row">
    ...
    <div class="field no-search">
        <input type="checkbox" name="mh_leavetype" value="VL" checked=""/> VACATIwrerON 
        <input type="checkbox" name="mh_leavetype" value="SL"/> SICK 
        <input type="checkbox" name="mh_leavetype" value="ABSENT"/>  ABSENT W/O PAY <!-- old was EMERGENCY -->
        <input type="checkbox" name="mh_leavetype" value="other"/> OTHER
        <select name="othleave" id="othleave" style="width: 150px;"><?=$this->employeemod->othLeave('',false);?></select> <!-- updated by justin (with e) for ica-hyperion 21194  -->
    </div>
</div>
<div class="form_row">
...
$("#button_save_modal").unbind("click").click(function(){  
    ...
    $.ajax({
        // url:"<?=site_url("process_/save_new_leave")?>",
        url:"<?=site_url("leave_/saveLeaveHRDirect")?>",
        type:"POST",
        data:{
            eid: $("#mh_empid").val(),
            ltype: $("input[name='mh_leavetype']").val(),
            othleave: $("select[name='othleave']").val(),
