...
$show   = false;
// $action = isset($this->input->post('action'))?$this->input->post('action'):"";
...
<div class="panel">
 <div class="panel-heading"><h4><b>SERVICE CREDIT APPLY LIST <?=$notif > 0 ? "<div class='notifdiv' style='color:black'><i class='glyphicon glyphicon-bell large'></i><span class='notifcount'><b>".$notif."</b></span></div>":""?></b></h4></div>
 <div class="panel-body">
<table class="table table-striped table-bordered table-hover" id="servicecredith">
    ...
    <?
        $query = $CI->service_credit->displayschistoryManagement($status,$dfrom,$dto);
        if($query->num_rows() > 0){
    ?>
    <tbody>
        <?
       $usage = "";
        ...
        <!-- <?=$data->dstatus?> -->
            <tr>
            <td>
              <?php if (($row->status != "DISAPPROVED" && $row->status != "APPROVED") && $row->hrstatus != "APPROVED"): ?>
              ...
              <?php endif ?>
                ...
                <td <?=$bold?> class='align_center'><?=$row->total_sc?($row->total_sc != $row->available_sc?"Used":"Un use"):"Un use"?></td>
                 <td class='align_center'>
                  ...
                  data-date="<?=$row->date?>" idkey="<?=$row->id?>" title="View Usage Details" ><i class="icon-large icon-eye-open"></i></a></td>
                 <?php
...
<script>
  ...
  $(document).ready(function(){
    var table = $('#servicecredith').DataTable({
    });
