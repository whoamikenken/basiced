...
class Employeemod extends CI_Model {
    ...
    function loadleavestatus($category="",$dfrom="",$dto="",$dept="",$cnoti=""){
        ...
        if($cnoti){
			if($dept == "HR")
			{
				$query = $this->db->query("SELECT a.*, b.htimefrom, b.htimeto ,CONCAT(c.lname,', ',c.fname,' ',c.mname) AS fullname , c.deptid
...
UNION
SELECT d.*,CONCAT(c.lname,', ',c.fname,' ',c.mname) AS fullname , c.deptid
...
LEFT JOIN leave_app e ON e.employeeid = a.employeeid
LEFT JOIN employee c ON e.employeeid = c.employeeid
WHERE FIND_IN_SET('$user',a.head) AND a.other <> 'DA' $wC AND DATE(e.timestamp) BETWEEN '$dfrom' AND '$dto' AND '$dto'
UNION
SELECT d.*,CONCAT(c.lname,', ',c.fname,' ',c.mname) AS fullname , c.deptid
FROM leave_app_hrd d
LEFT JOIN leave_app e ON e.employeeid = d.employeeid
LEFT JOIN employee c ON e.employeeid = c.employeeid
				WHERE FIND_IN_SET('$user',d.head) AND d.other <> 'DA' $wC2 AND DATE(e.timestamp) BETWEEN '$dfrom' AND '$dto' AND '$dto' ");
			}
			else
			{
        $query = $this->db->query("SELECT a.*, CONCAT(b.lname,', ',b.fname,' ',b.mname) as fullname, b.deptid FROM $tbl a
    ...
    function loadseminarappdata($id="",$manage=false){
        ...
        $query = $this->db->query("SELECT *,a.`employeeid` AS eid ,c.description AS epos, d.description AS edept FROM $tbl a 
                                     LEFT JOIN employee b ON a.employeeid = b.employeeid
...
LEFT JOIN employee c ON e.employeeid = c.employeeid
    ...
    function manageleavenotif(){
		...
		else
		{
			...
			$query = $this->db->query("SELECT a.*, b.htimefrom, b.htimeto
FROM leave_app_dhead a
LEFT JOIN leave_app_hrd b ON a.aid = b.aid 
LEFT JOIN leave_app e ON e.employeeid = a.employeeid
WHERE FIND_IN_SET('$user',a.head) AND a.other <> 'DA' AND a.status = 'PENDING'
...
// echo "<pre>";print_r($leave_setup_data);die;
    ...
    function leave_approve_head($data){
        ...
        if($data['id']){
            ...
            for($seqno = $seqnum; $seqno< count($seq); $seqno++){
				...
				if($this->db->query("SELECT * from $tbl WHERE employeeid='{$eid}' AND aid='{$aid}' AND status='PENDING' ")->num_rows() > 0)
				{
					break;
				}
            } 
    ...
    function seminar_approve_head_withsequence($data){
        ...
        $seq = $this->getSeminarSequence('OBS',$deptid,'OBS');
        // echo "<pre>";print_r($seq);
        $seqnum = 1;
        for($seqno = $seqnum; $seqno< count($seq); $seqno++){
            if(in_array($user, explode(",",$seq[$seqno]["head"]))){
        ...
        if($seqnum == count($seq)){
		   ...
		   if($status == "DISAPPROVED"){
				...
				$query = $this->db->query("UPDATE seminar_app SET $head_col_status='$status', $head_col_date=CURRENT_DATE, isread=0, coursefee_approved = '{$data['coursefeeApproved']}',coursefee_approved = '{$data['coursefeeApproved']}' , meal_approved = '{$data['mealApproved']}' , transportation_approved= '{$data['transportationApproved']}' , hotel_approved = '{$data['hotelApproved']}' , othermiscellaneous_approved = '{$data['othermiscellaneousApproved']}' , totalcost_approved = '{$data['tcApproved']}' WHERE id='$aid'");
	...
	function hrunconfirmatt($data){
		...
		$query = $this->db->query("DELETE FROM `attendance_confirmed_nt` WHERE `employeeid` = '{$empid}' AND `cutoffstart` = '{$from_date}' AND `cutoffend` = '{$to_date}'");
