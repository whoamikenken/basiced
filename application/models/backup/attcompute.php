...
class Attcompute extends CI_Model {
    ...
    function displayLogTime($eid="",$date="",$tstart="",$tend="",$tbl="",$seq=1){
		...
		$query = $this->db->query("SELECT timein,timeout,otype FROM $tbl WHERE userid='$eid' AND DATE(timein)='$date' AND TIME(timein)<='$tend' ORDER BY timein ASC LIMIT $seq");// AND TIME(timein)<='$tend'
        #$query = $this->db->query("SELECT timein,timeout,otype FROM timesheet WHERE userid='$eid' AND DATE(timein)='$date' AND TIME(timein)<='$tend' AND (TIMEDIFF('$tend', TIME(timein)) > '00:30:00' AND TIMEDIFF('$tstart', TIME(timein)) < '01:00:00' ) ORDER BY timein DESC LIMIT 1");
        $seq = $seq - 1;
		if($query->num_rows() > 0){
        	...
        	if($query->num_rows() > 0){
	            $timein  = $query->row($seq)->localtimein;
	            $return = array($timein,"","");
            }else
                $return = array("","",true);
        }
        
        $query = $this->db->query("SELECT * FROM leave_app WHERE employeeid='$eid' AND '$date' BETWEEN datefrom AND dateto AND hrdirstatus='APPROVED'");
        if($query->num_rows() > 0){
			// if($query->row($seq)->paid == "YES")
			// {
				$return = array($query->row($seq)->timefrom,$query->row($seq)->timeto,"");
			// }
			// else
			// {
				// $return = array($query->row($seq)->timefrom,$query->row($seq)->timeto,true);
			// }
		}
