...
<?
    $x = $tlec = $tlab = $tabsent = $tel = $tvl = $tsl = $tol = $tdlec = $tdlab = $tholiday = $tempOverload = $overload = $tOverload = $lastDayOfWeek = $firstDayOfWeek = ""; 
	$tempabsent = "";
    ...
    if($countrow > 0){
        ...
        <tr class="edata" <?=$color?>>
            ...
            <?if($dispLogDate){?>
                ...
                <td class="align_center" rowspan="<?=$countrow?>" key="ol"><?=($ol ? ($oltype ? ($oltype == "ABSENT" ? "ABSENT W/ FILE" : $oltype) : $this->employeemod->othLeaveDesc($ol)) : ($q ? ($q == "1" ? "" : $q) : ""))?> <?=$holiday?$holidayInfo['description']:""?></td>
            <?}?>
...
<?}?>
				...
				<td class="align_center" key="to" style=""><?=$logout?date("h:i A",strtotime($logout)):"--"?></td>
<?if($stime && $etime){?>
				<td class="align_center" rowspan="<?=count($log)?>" key="overload">--</td>
				<td class="align_center" rowspan="<?=count($log)?>" key="subtitute">--</td>
				<td class="align_center" rowspan="<?=count($log)?>" key="utlec">--</td>
				<td class="align_center" rowspan="<?=count($log)?>" key="utlab">--</td>
				...
				<td class="align_center" rowspan="<?=count($log)?>" key="ol"><?=($ol ? ($oltype ? ($oltype == "ABSENT" ? "ABSENT W/ FILE" : $oltype) : $this->employeemod->othLeaveDesc($ol)) : ($q ? ($q == "1" ? "" : $q) : ""))?> <?=$holiday?$holidayInfo['description']:""?></td>
				<?}?>
...
?>
			<tr class="edata" style='background-color: gray;color:white'>
				...
				<td class="align_center" key="ol"><?=($ol ? ($oltype ? ($oltype == "ABSENT" ? "ABSENT W/ FILE" : $oltype) : $this->employeemod->othLeaveDesc($ol)) : "")?> <?=$holiday?$holidayInfo['description']:""?></td>
				<td class="align_center" key="dlec">--</td>
				<td class="align_center" key="dlab">--</td>
				<td class="align_center"><?=$holiday?$holidayInfo['type']:"--"?></td>
			</tr>
<?
		}
			...
			if($rsched->flexible != "YES")
			{
				...
				<tr class="edata" <?=$color?>>
					...
					<?if($dispLogDate){?>
						...
						<td class="align_center" rowspan="<?=$countrow?>" key="vl"><?=$vl?></td>
						<td class="align_center" rowspan="<?=$countrow?>" key="sl"><?=$sl?></td>
						<td class="align_center" rowspan="<?=$countrow?>" key="ol" style="<?=($pending)?'background-color:#b3d9ff;':''?>"><?=($pending)?"PENDING ".$pending:($ol ? ($oltype ? ($oltype == "ABSENT" ? "ABSENT W/ FILE" : $oltype) : $this->employeemod->othLeaveDesc($ol)) : ($q ? ($q == "1" ? "" : $q) : ""))?> <?=$service_credit?"SERVICE CREDIT":(!is_null($service_credit)?"SERVICE CREDIT":"")?></td><!--//($ol ? ($oltype=="DIRECT" ? $this->employeemod->othLeaveDesc($ol) : $oltype) : "")?>-->
					<?}?>
					<?if($dispLogDate){?>
						<td class="align_center" rowspan="<?=$countrow?>"><?=$holiday?$holidayInfo['type']:""?></td>
					<?}?>
