...
class Attendance_ extends CI_Controller {
	...
	public function loadAttendanceReport(){
        ...
        $arr_empids = $this->utils->getEmployeeIDList($deptid_post,'',$tnt,$empid_post,' ORDER BY campus, department, fullname', $campus, $deptid, $office);
...
// echo "<pre>"; print_r($date_range); die;
	...
	public function getDailyAttendanceDetails($empid='', $date='',$edata='',$hasLog='',$isSuspension='',$holiday='',$tnt='teaching',$fixedday=TRUE){
		$perday_info = array();
	...
	public function getPerSchedAtendanceDetails($empid='',$date='',$edata='',$rsched='',$sched_seq=0,$isCreditedHoliday=false,$holiday='',$tnt='teaching',$fixedday=TRUE,$hasLog=false){
		$persched_info = array();
		// $sched_seq++;
		$hasleavecount = 0;
		$haswholedayleave = false;
    ...
    public function validateConfirmAttendance(){
     ...
     public function validateAttendanceConfirmedViewing(){
        ...
        if($tnt == "teaching"){
            $data['result'] = $this->viewAttendanceConfirmed($dfrom, $dto, $tnt,$employeeid, array(), $campus, $deptid, $office);
            $this->load->view("employeemod/viewattconfirm", $data);
        }else{

            $data['result'] = $this->viewAttendanceConfirmedNT($dfrom, $dto, $tnt,$employeeid, array(), $campus, $deptid, $office);
            $this->load->view("employeemod/viewattconfirm_nt", $data);
        }
    }

    public function viewAttendanceConfirmed($dfrom, $dto, $tnt,$employeeid,$emplist=array(), $campus="", $deptid = "", $office = ""){
        $totDeduction = $totdeduc = 0;
        $att_list = $this->attendance->emp_confirmed($dfrom, $dto, $tnt,$employeeid, $campus, $deptid, $office);
    ...
    public function viewAttendanceConfirmedNT($dfrom, $dto, $tnt,$employeeid,$emplist=array(), $campus="", $deptid = "", $office = ""){
        $totDeduction = $totdeduc = 0;
        $att_list = $this->attendance->emp_confirmed_nt($dfrom, $dto, $tnt,$employeeid, $campus, $deptid, $office);
        ...
        foreach($att_list as $row){
            ...
            // $tutlec = $row["ut"];
            $tabsent = $row["absent"];
            // $lateutdeduc = $this->attcompute->exp_time($tlec) + $this->attcompute->exp_time($tutlec);
    ...
    public function loadCutoffAttendance_Summary(){
        ...
        ///< get data
        $data['cutoff'] = $cutoff;
        $data['attendance_list'] = $this->hr_reports->getAttConfirmed_summary($teachingtype,$sdate,$edate,$payroll_start,$employeeid,$campus,$deptid);
        $this->load->model('utils');
    ...
    public function getAttendanceToday(){
        ...
        $active_employee = $this->extensions->getActiveEmployees($where_clause);
        ...
        $leave_employee = $this->leave->getLeaveTodayEmployees($datenow, $where_clause);
        if($leave_employee){
        ...
        $ob_employee = $this->ob_application->getObTodayEmployees($datenow, $where_clause);
        if($ob_employee){
