...
function getJobQualification(){
    ...
    function saveToAllCardNewEmployee($employeeid){
		...
		$empinfo = array(
			...
			"CampusName" => "Saint Pedro Poveda College",
    ...
    function saveApplicantStatus(){
    	...
    	if($result) $response["msg"] = "Successfully saved applicant status";
    	else $response["msg"] = "Failed to saved applicant status";
	...
	function removeApplicantDoc(){
		...
		$res = $this->applicantt->removeApplicantDoc($where_clause);
	...
	function saveApplicantFilledForm()
	{
        ...
        if ($usertype != "ADMIN")  unset($data["dra_remarks"]);
	...
	function checkIfHasData(){
		...
		list($response['isexist'], $response['seqno'], $response['submitted'], $response['email'], $response['redtag'], $response['isactive']) = $this->applicantt->checkIfHasData($lname, $mname, $fname, $email, $positionid);
		echo json_encode($response);
	}
